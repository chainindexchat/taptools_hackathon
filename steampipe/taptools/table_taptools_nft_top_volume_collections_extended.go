...
type TopVolumeCollectionsExtendedResponse struct {
...
func listTopVolumeCollectionsExtended(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	...
	reqUrl := "https://openapi.taptools.io/api/v1/nft/top/volume/extended"
	if timeframe != "" {
	...
	var apiResponse []TopVolumeCollectionsExtendedResponse
	...
	for _, item := range apiResponse {
		d.StreamListItem(ctx, TopVolumeCollectionsExtendedResponse{
			Listings:       item.Listings,
			ListingsPctChg: item.ListingsPctChg,
			Logo:           item.Logo,
			Name:           item.Name,
			Owners:         item.Owners,
			OwnersPctChg:   item.OwnersPctChg,
			Policy:         item.Policy,
			Price:          item.Price,
			PricePctChg:    item.PricePctChg,
			Sales:          item.Sales,
			SalesPctChg:    item.SalesPctChg,
			Supply:         item.Supply,
			Volume:         item.Volume,
			VolumePctChg:   item.VolumePctChg,
			Timeframe:      timeframe,
			Page:           page,
			PerPage:        perPage,
		})
...
func tableTaptoolsTopVolumeCollectionsExtended() *plugin.Table {
	return &plugin.Table{
		Name:        "taptools_top_volume_collections_extended",
		Description: "Get top NFT collections by trading volume over a given timeframe, including percentage changes.",
		List: &plugin.ListConfig{
			Hydrate: listTopVolumeCollectionsExtended,
