...
type NFTVolumeTrendedResponse struct {
...
func listNFTVolumeTrended(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	...
	reqUrl := fmt.Sprintf("https://openapi.taptools.io/api/v1/nft/collection/volume/trended?policy=%s&interval=%s", policy, interval)
	if numIntervals != 0 {
	...
	var apiResponse []NFTVolumeTrendedResponse
	...
	for _, item := range apiResponse {
		d.StreamListItem(ctx, NFTVolumeTrendedResponse{
			Price:        item.Price,
			Sales:        item.Sales,
			Time:         item.Time,
			Volume:       item.Volume,
...
func tableTaptoolsNFTVolumeTrended() *plugin.Table {
	return &plugin.Table{
		Name:        "taptools_nft_volume_trended",
		Description: "Get trended volume and number of sales for a particular NFT collection.",
		List: &plugin.ListConfig{
			Hydrate: listNFTVolumeTrended,
		...
		Columns: []*plugin.Column{
			// Response properties
			{Name: "price", Type: proto.ColumnType_DOUBLE, Description: "Average price for the interval"},
			{Name: "sales", Type: proto.ColumnType_INT, Description: "Number of sales during the interval"},
