...
func listActiveListings(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	...
	reqUrl := fmt.Sprintf("https://openapi.taptools.io/api/v1/nft/collection/listings?policy=%s", policy)
	...
	d.StreamListItem(ctx, ActiveListingsResponse{
...
func tableTaptoolsActiveListings() *plugin.Table {
	return &plugin.Table{
		...
		Columns: []*plugin.Column{
			// Response properties
			{Name: "listings", Type: proto.ColumnType_INT, Description: "Number of active listings in the collection"},
